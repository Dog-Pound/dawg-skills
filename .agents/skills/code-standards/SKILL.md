...
2. **Decide before code.** Resolve every material decision against repository evidence. Apply domain fit, authority, ownership, lifecycle, then reason to change. Apply governing patterns and ADRs. A non-empty frontier stops here for the repository's required planning route; Plan begins after the user confirms shared understanding. Done when every material choice is resolved or explicitly returned to the user.
3. **Plan a production tracer.** Define the smallest production-shaped vertical slice that proves the decision. Declare its behavior, owning deep modules, public contracts, outcomes, effects, causal proof, acceptance criteria, and filesystem contract. Done when every behavior, seam, outcome, effect, proof, acceptance criterion, and affected file is declared.
